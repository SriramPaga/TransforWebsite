...
const MultipleInputField = ({ setSelected, options, selected }) => {
    ...
    <div className="sm:col-span-6 d-flex">
      <div className="bg-gray-200 p-4 rounded-md ">
        <div className="relative  text-sm">
          {selected?.length ? (
            <div className="bg-white w-auto relative text-xs flex flex-wrap gap-1 p-2 mb-2">
              {selected.map((tag) => {
                return (
                  <div
                    key={tag}
          ...
          {/* Menu's */}
          {menuOpen ? (
            <div className="bg-slate-600 card absolute w-full max-h-52 mt-2 p-1 flex overflow-y-auto scrollbar-thin scrollbar-track-slate-50 scrollbar-thumb-slate-200">
              <ul className="w-full">
                {filteredTags?.length ? (
